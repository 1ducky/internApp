...
async function getPublicProfile(userId: string) {
    logger.info("Public profile retrieval request received", 'getPublicProfile')
    const res = await profileRepository.getPublicProfileById(userId)
    if (!res.success || !res.data) {
        logger.error("Public profile retrieval failed", 'getPublicProfile')
        return null
    }
    logger.info("Public profile retrieval success", 'getPublicProfile')
    return toPublicProfileDto(res)
}